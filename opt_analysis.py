...
def main():
    ...
    for index, dist in enumerate(med_y):
        if index % bucket_size == bucket_size - 1:
            ...
            avg_y_med_fut_dist.append(np.median(temp_y_med_lst))
    ...
    plt.savefig('img/fut_rd_stats_' + file_name + '_size_' + str(cache_size) + '.png')
    plt.close()
    ...
    axs[1].set(xlabel='Lags (Virtual Time)')
    axs[0].set_title('Future RD Median Auto-Correlation')
    axs[1].set_title('Future RD Q3 Auto-Correlation')
    fig.savefig('img/fut_rd_med_q3_autocorr_' + file_name + '_size_' + str(cache_size) + '.png')
    ...
    plt.savefig('img/evict_dist_stats_' + file_name + '_size_' + str(cache_size) + '.png')

    t6 = time.time()
    print('Time for Average Eviction Distance Plot: ', str(t6-t4)) 
